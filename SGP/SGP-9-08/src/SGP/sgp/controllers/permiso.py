...
class BusquedaTableFiller(TableFiller):
    ...
    def permisos(self, obj, **kw):
        permiso = kw['permiso']
        permisos = ', '.join(['<a href="/permisos/'+str(d.nombre)+'">'+d.descripcion+'</a>'
                               for d in obj.permisos if d!=permiso])
        return permisos.join(('<div>', '</div>'))
...
#        pm = PermisoManager()
#        params = kw
#        lista = pm.buscar(params["parametro"])
#        tmpl_context.widget = self.table
#        value = self.table_filler.get_value()
#        return dict(permiso=lista, value_list=value, model="permiso")
# ################################################################################################       
    @with_trailing_slash
    @expose('sgp.templates.get_all_permiso')
    @expose('json')
